...
#error "Please define a way to initialize a memory block."
	...
	ee_printf("Total ticks      : %llu\n\r", (long long unsigned)total_time);;
	ee_printf("Total ticks      : %llu\n\r", (long long unsigned)results[0].iterations);
#if HAS_FLOAT
	ee_printf("Total time (secs): %d\n\r", (int)time_in_secs(total_time));
	ee_printf("Resutls: %d\n\r", (int)( results[0].iterations
	...
	//xil_printf("[0x%04X]crcfinal      : 0x%04x\n\r", core_count, results[0].crc);
    Xil_Out16(0x80000000,(results[0].crc)+core_count);
	core_count++;
